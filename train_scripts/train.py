    ...
    def run_sampling(init_z=None, label_suffix="", vae=None, sampler="dpm-solver"):
        ...
        for prompt in validation_prompts:
            z = (
                torch.randn(
                    1,
                    3 if pixel_space else config.vae.vae_latent_dim,
                    latent_size,
                    latent_size,
                    device=device,
                )
        ...
        torch.cuda.empty_cache()
        if not pixel_space and vae is None:
        ...
        for prompt, latent in zip(validation_prompts, latents):
            samples = latent if pixel_space else vae_decode(config.vae.vae_type, vae, latent.to(vae_dtype))
            samples = (
...
def train(
    ...
    loss_nan_timer = 0
    eval_sampling_epochs = getattr(config.train, "eval_sampling_epochs", None)
    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if pixel_space:
                z = batch[0].to(accelerator.device, non_blocking=True)
            elif load_vae_feat:
            ...
            with accelerator.accumulate(model):
                ...
                optimizer.step()
                lr_scheduler.step()
                # AcceleratedOptimizer only clears gradients on synchronized
                # update steps, preserving all preceding accumulation steps.
                optimizer.zero_grad()
                accelerator.wait_for_everyone()
            ...
            if not torch.isfinite(loss):
                raise ValueError(f"Non-finite loss at global_step={last_completed_step}: {loss.item()}")
            reached_max_train_steps = (
                config.train.max_train_steps is not None and run_steps >= config.train.max_train_steps
            )
            if (
                last_completed_step % config.train.save_model_steps == 0
                or (
                    config.train.early_stop_hours > 0
                    and (time.time() - training_start_time) / 3600 > config.train.early_stop_hours
                )
            ):
                ...
                if accelerator.is_main_process:
                    if (
                        ...
                        and last_completed_step % config.train.eval_metric_step == 0
                        and step > 1
                    ):
                ...
                if (
                    ...
                    logger.info(f"Stopping training at epoch {epoch}, step {last_completed_step} due to time limit.")
                    return

            if eval_sampling_epochs is not None and eval_sampling_epochs > 0:
                # Epoch-based validation runs after the epoch-level checkpoint below.
                should_validate = False
            else:
                should_validate = (
                    last_completed_step % config.train.eval_sampling_steps == 0 or (step + 1) == 1
                )
            if config.train.visualize and should_validate:
                if config.train.use_fsdp:
            ...
            if reached_max_train_steps:
                logger.info("Reached max_train_steps=%d; stopping cleanly.", config.train.max_train_steps)
                return

            # avoid dead-lock of multiscale data batch sampler
            if (
                config.model.multi_scale
                and config.data.max_samples is None
                and train_dataloader_len >= 30
        ...
        if (
            ...
            and eval_sampling_epochs is not None
            and eval_sampling_epochs > 0
            and epoch % eval_sampling_epochs == 0
        ):
            accelerator.wait_for_everyone()
            if accelerator.is_main_process:
                log_validation(
...
def main(cfg: SanaConfig) -> None:
    ...
    global image_size, cache_file, total_steps, vae_dtype, model_instance, pixel_space

    config = cfg
    args = cfg
    pixel_space = getattr(config.data, "data_space", "latent") == "pixel"
    if getattr(config.data, "data_space", "latent") not in {"latent", "pixel"}:
        raise ValueError(f"Unsupported data.data_space={config.data.data_space!r}")
    ...
    latent_size = image_size if pixel_space else int(image_size) // config.vae.vae_downsample_rate
    ...
    if pixel_space and config.data.load_vae_feat:
        raise ValueError("pixel data_space requires RGB images; data.load_vae_feat must be false")
    if not pixel_space and not config.data.load_vae_feat:
        vae = get_vae(config.vae.vae_type, config.vae.vae_pretrained, accelerator.device).to(vae_dtype)
    ...
    if pixel_space:
        logger.info("data space: pixel (VAE construction, encode, and decode are disabled)")
    else:
    ...
    text_encoder_id = osp.basename(config.text_encoder.text_encoder_name.rstrip("/"))
    null_embed_path = osp.join(
        config.train.null_embed_root,
        f"null_embed_diffusers_{text_encoder_id}_{max_length}token_{text_embed_dim}.pth",
    )
    ...
    if config.train.visualize and len(config.train.validation_prompts):
        valid_prompt_embed_suffix = f"{max_length}token_{text_encoder_id}_{text_embed_dim}.pth"
    ...
    model_data_space = getattr(model, "data_space", "latent")
    if pixel_space != (model_data_space == "pixel"):
        raise ValueError(
            f"data/model space mismatch: data.data_space={config.data.data_space!r}, model={model.__class__.__name__}"
        )
    ...
    if config.model.load_from is not None and load_from:
        ...
        if pixel_space:
            ...
            disallowed_missing = sorted(set(missing).difference(allowed_missing))
            if disallowed_missing or unexpected:
                raise RuntimeError(
                    "pixel initialization checkpoint failed strict audit: "
    ...
    if pixel_space:
        ...
        logger.info(
            "L2P trainable boundary: first=%d, last=%d, trainable=%s, frozen=%s, total=%s",
            config.model.l2p_first_trainable_blocks,
            config.model.l2p_last_trainable_blocks,
            f"{freeze_report['trainable']:,}",
            f"{freeze_report['frozen']:,}",
            f"{freeze_report['total']:,}",
        )
    ...
    if config.data.max_samples is not None:
        if config.data.max_samples <= 0:
            raise ValueError(f"data.max_samples must be positive, got {config.data.max_samples}")
        sampler_num_samples = min(config.data.max_samples, len(dataset))
        if sampler_num_samples < num_replicas:
            raise ValueError(
                f"data.max_samples={sampler_num_samples} must be >= world size {num_replicas}"
            )
        logger.warning("Limiting this diagnostic run to the first %d dataset samples.", sampler_num_samples)
    accelerator.wait_for_everyone()
    if config.model.multi_scale:
        ...
        base_pattern = (
            ...
            f"-max_samples{sampler_num_samples}"
        ...
        sampler = DistributedRangedSampler(
            dataset, num_replicas=num_replicas, num_samples=sampler_num_samples, rank=rank
        )
    ...
    if config.train.lr_schedule_args and config.train.lr_schedule_args.get("num_warmup_steps", None):
        warmup_updates = config.train.lr_schedule_args["num_warmup_steps"]
        # Accelerate advances a prepared scheduler once per process when
        # split_batches=False, so scale the internal count to preserve the
        # configured number of global optimizer updates.
        config.train.lr_schedule_args["num_warmup_steps"] = warmup_updates * num_replicas
        logger.info(
            "Warmup: %d global optimizer updates (%d internal scheduler steps across %d processes).",
            warmup_updates,
