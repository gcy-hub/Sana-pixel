...
def set_env(seed=0, latent_size=256, channels=4):
    torch.manual_seed(seed)
    torch.set_grad_enabled(False)
    for _ in range(30):
        torch.randn(1, channels, latent_size, latent_size)
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), desc=tqdm_desc, unit="batch", position=args.gpu_id, leave=True):
        ...
        if bs == 1:
            ...
            latent_size_h, latent_size_w = (
                (int(hw[0, 0]), int(hw[0, 1]))
                if pixel_space
                else (int(hw[0, 0] // config.vae.vae_downsample_rate), int(hw[0, 1] // config.vae.vae_downsample_rate))
        ...
        with torch.no_grad():
            ...
            if args.sampling_algo == "dpm-solver":
                ...
                samples = sa_solver.sample(
                    S=25,
                    batch_size=n,
                    shape=(3 if pixel_space else config.vae.vae_latent_dim, latent_size_h, latent_size_w),
                    eta=1,
        ...
        if not pixel_space:
            samples = vae_decode(config.vae.vae_type, vae, samples.to(vae_dtype))
...
if __name__ == "__main__":
    ...
    if pixel_space:
        if not config.model.load_from:
            raise ValueError("pixel inference requires model.load_from in the config")
        args.model_path = config.model.load_from
    ...
    set_env(
        args.seed,
        args.image_size if pixel_space else args.image_size // config.vae.vae_downsample_rate,
        channels=3 if pixel_space else config.vae.vae_latent_dim,
    )
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger = get_root_logger()

    # only support fixed latent size currently
    latent_size = args.image_size if pixel_space else args.image_size // config.vae.vae_downsample_rate
    ...
    vae = None if pixel_space else get_vae(config.vae.vae_type, config.vae.vae_pretrained, device).to(vae_dtype)
    ...
    if pixel_space:
        disallowed_missing = sorted(set(missing).difference({"pos_embed"}))
        if disallowed_missing or unexpected:
            raise RuntimeError(
                "pixel checkpoint failed strict audit: "
