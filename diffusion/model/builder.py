...
def get_tokenizer_and_text_encoder(name="T5", device="cuda"):
    ...
    if os.path.isdir(os.path.expanduser(name)):
        ...
        model_path = text_encoder_dict[name]
    if "T5" in name:
        tokenizer = T5Tokenizer.from_pretrained(model_path)
        text_encoder = T5EncoderModel.from_pretrained(model_path, torch_dtype=torch.float16).to(device)
    elif "gemma" in name:
        ...
        text_handler = QwenVLEmbedder(model_id=model_path, device=device)
