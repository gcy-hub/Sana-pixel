...
class DataConfig(BaseConfig):
    ...
    # ``latent`` preserves the original SANA behavior. ``pixel`` feeds the
    # normalized RGB tensor directly to a pixel-space model and never builds a VAE.
    data_space: str = "latent"
    ...
    hq_only: bool = False
    valid_num: int = 0
    # Optional deterministic prefix used only by staged overfit/pilot runs.
...
class ModelConfig(BaseConfig):
    ...
    l2p_first_trainable_blocks: int = 5
    l2p_last_trainable_blocks: int = 5
    extra: Any = None
...
class TrainingConfig(BaseConfig):
    ...
    early_stop_hours: float = 100  # Set to 0 to disable the training-time limit.
    num_epochs: int = 100
    ...
    # If set, validation runs on the first batch of every Nth epoch and
    # takes precedence over eval_sampling_steps.
    eval_sampling_epochs: Optional[int] = None
    lora_rank: int = 4
    log_interval: int = 50
    mask_type: str = "null"
...
def model_init_config(config: SanaConfig, latent_size: int = 32):
    ...
    data_space = getattr(config.data, "data_space", "latent")
    if data_space not in {"latent", "pixel"}:
        raise ValueError(f"data.data_space must be 'latent' or 'pixel', got {data_space!r}")
    pixel_space = data_space == "pixel"
    return {
        "input_size": config.model.image_size if pixel_space else latent_size,
        ...
        "in_channels": 3 if pixel_space else config.vae.vae_latent_dim,
